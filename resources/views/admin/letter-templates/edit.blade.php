    ...
    <div class="py-4">
        <div class="container">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.letter_templates.update', $template->id) }}">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label class="form-label">Jenis Surat</label>
                            <input type="text" class="form-control" value="{{ strtoupper(str_replace('_',' ', $template->letter_type)) }}" disabled>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="body" class="form-control" rows="5" placeholder="Gunakan placeholder: {nama}, {nim}, {prodi}, {judul_skripsi}, {tahun_lulus}">{{ old('body', $template->body) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Footer (opsional)</label>
                            <textarea name="footer" class="form-control" rows="3">{{ old('footer', $template->footer) }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <a href="{{ route('admin.letter_templates.index') }}" class="btn btn-outline-secondary">Kembali</a>
